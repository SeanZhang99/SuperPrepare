...
def run_dist(
    ...
    os.environ["MASTER_PORT"] = f"123{port_postfix:02d}"
    ...
    #     L2Factor = config["train"]["L2_Factor"]
    #     numEpoch = config["train"]["epochs"]

    #     className = datasets[0].getClassName()

    #     net = model_name(**model_args)

    #     net = DDP(net.to(rank))

    #     cp_path = tb_filename + "/model.cp"
    #     if rank == 0:
    #         torch.save(net.state_dict(), cp_path)
    #     dist.barrier()
    #     net.load_state_dict(
    #         torch.load(cp_path, map_location=f"cuda:{rank}", weights_only=True)
    #     )

    #     net = net.to(rank)

    #     trainWeight = datasets[0].getClassWeight()
    #     trainWeight = 1 / (trainWeight + 1e-8)
    #     trainWeight = trainWeight.to(rank)
    #     testWeight = datasets[2].getClassWeight()

    #     use_train_weight = True
    #     if trainWeight.size(dim=0) == numCategories and use_train_weight:
    #         lossfn = nn.CrossEntropyLoss(weight=trainWeight, reduction="mean")
    #     else:
    #         lossfn = nn.CrossEntropyLoss(reduction="mean")
    #     decayed_p, default_p = [], []
    #     recursivelyAddParametersToGroups(net, decayed_p, default_p)
    ...
    #     optimizer = NoamOpt(
    #         optimizer=optimizer,
    #         max_lr={"default": learningRate},
    #         warmup_step=config["train"]["warmup_step"],
    #         lr_reduce_factor={"default": config["train"]["lr_reduce_factor"]},
    #         patience=config["train"]["lr_reduce_patience"],
    #         cooldown=config["train"]["lr_reduce_cooldown"],
    #         min_lr={"default": config["train"]["min_lr"]},
    #     )
